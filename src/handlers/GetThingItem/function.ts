...
import { ThingItemKeys, ThingItemData, getKeys } from '../../lib/ThingItem.js'
 ...
 */
export async function getItem(itemKeys: ThingItemKeys): Promise<ThingItemData> {
...
export async function handler (event: APIGatewayProxyEvent, _: Context): Promise<APIGatewayProxyResult> {
    ...
    let body: string
    try {
        const itemData: ThingItemData = await getItem(
            itemKeys,
        )
        statusCode = 200
